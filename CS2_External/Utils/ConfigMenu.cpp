...
namespace ConfigMenu {
    ...
    void RenderConfigMenu() {
		// Config
		if (ImGui::BeginTabItem(u8"配置文件 "))
		{
			static char configNameBuffer[128] = "";

			ImGui::InputText(u8"新建文件名", configNameBuffer, sizeof(configNameBuffer));
			...
			if (selectedConfig != -1)
			{
				ImGui::Text(u8"选中文件: %s", configFiles[selectedConfig].c_str());
			}

			if (ImGui::Button(u8"加载选中") && selectedConfig >= 0 && selectedConfig < configFiles.size())
			{
				std::string selectedConfigFile = configFiles[selectedConfig];
				MyConfigSaver::LoadConfig(selectedConfigFile);
			}

			if (ImGui::Button(u8"保存选中") && selectedConfig >= 0 && selectedConfig < configFiles.size())
			{
				std::string selectedConfigFile = configFiles[selectedConfig];
			...
			if (ImGui::Button(u8"删除选中") && selectedConfig >= 0 && selectedConfig < configFiles.size())
			{
				std::string selectedConfigFile = configFiles[selectedConfig];
